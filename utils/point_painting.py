...
def point_painting(lidar, sems, coord_converters):
    ...
    sem_h, sem_w, sem_c = sems[0].shape
    
    lidar_painted = np.concatenate([lidar, np.zeros((len(lidar), sem_c))],axis=-1)

    for sem, coord_converter in zip(sems, coord_converters):

        lidar_cam = coord_converter.lidar_to_cam(lidar)
        lidar_cam_u, lidar_cam_v = map(lambda x: x[...,0], np.split(lidar_cam, 2, axis=-1))
        valid_idx = (lidar_cam_u>=0)&(lidar_cam_u<sem_w)&(lidar_cam_v>=0)&(lidar_cam_v<sem_h)
        lidar_cam = lidar_cam[valid_idx]

        lidar_sem = sem[lidar_cam[...,1],lidar_cam[...,0]]

        lidar_painted[valid_idx,lidar_d:] = lidar_sem
