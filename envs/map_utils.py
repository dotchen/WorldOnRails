...
class MapImage(object):
    def __init__(self, carla_world, carla_map, pixels_per_meter, show_triggers, show_connections, show_spawn_points):
        ...
        self.draw_road_map(
                self.big_map_surface, self.big_lane_surface, self.big_waypoint_surfaces,
    ...
    def draw_road_map(self, map_surface, lane_surface, waypoint_surfaces, carla_world, carla_map, world_to_pixel, world_to_pixel_width, threshold=math.radians(1)):
        ...
        def draw_traffic_signs(surface, font_surface, actor, color=COLOR_ALUMINIUM_2, trigger_color=COLOR_PLUM_0):
            ...
            pygame.draw.lines(surface, color, True, line_pixel, 1)

            # draw bounding box
            if self.show_triggers:
                corners = Util.get_bounding_box(actor)
                corners = [world_to_pixel(p) for p in corners]
                pygame.draw.lines(surface, trigger_color, True, corners, 1)
...
class ModuleWorld(object):
    ...
    def _render_traffic_lights(self, surface, list_tl, world_to_pixel, world_to_pixel_width):
        ...
        for tl in list_tl:
            ...
            o = tl.get_transform().rotation.yaw
            v = np.array([
                np.cos(np.radians(o)),
                np.sin(np.radians(o))])

            angle = tl.get_transform().rotation.yaw
    ...
    def _render_walkers(self, surface, list_w, world_to_pixel, world_to_pixel_width):
        for w in list_w:
            ...
            # if True:
                ...
                radius = world_to_pixel_width(bb.x + self.hero_x)
    ...
    def _render_vehicles(self, surface, list_v, world_to_pixel):
        ...
        for v in list_v:
            ...
            corners = [carla.Location(x=-bb.x-self.hero_x-2, y=-bb.y-self.hero_y/2),
                      carla.Location(x=-bb.x-self.hero_x-2, y=bb.y+self.hero_y/2),
                      carla.Location(x=bb.x+1, y=bb.y+self.hero_y/2),
                      carla.Location(x=bb.x+1, y=-bb.y-self.hero_y/2)
                      ]
            # corners = [carla.Location(x=-bb.x, y=-bb.y),
            #           carla.Location(x=-bb.x, y=bb.y),
    ...
    def render(self, display):
        ...
        if self.hero_actor is not None:
            ...
            # map_pivot = self.hero_map_surface.get_rect(center=center)
            # lane_pivot = self.hero_lane_surface.get_rect(center=center)
            # vehicle_pivot = self.hero_vehicle_surface.get_rect(center=center)
            # walker_pivot = self.hero_walker_surface.get_rect(center=center)
            # traffic_pivot = self.hero_traffic_light_surface.get_rect(center=center)
            # # rotation_self_pivot = rotated_self_surface.get_rect(center=center)

            # self.window_map_surface.blit(self.hero_map_surface, map_pivot)
            # self.window_lane_surface.blit(self.hero_lane_surface, lane_pivot)
            # self.window_vehicle_surface.blit(self.hero_vehicle_surface, vehicle_pivot)
            # self.window_walker_surface.blit(self.hero_walker_surface, walker_pivot)
            # self.window_traffic_light_surface.blit(
            #         self.hero_traffic_light_surface, traffic_pivot)
            ...
            #     pygame.surfarray.array3d(self.hero_traffic_light_surface),
            # 0, 1)
...
class Wrapper(object):
    ...
    def get_observations(cls):
        road, lane, vehicle, pedestrian, traffic, waypoints = cls.world_module.get_rendered_surfaces()
